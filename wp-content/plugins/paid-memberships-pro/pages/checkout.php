...
$user_id = get_current_user_id();
	...
	$go_url=site_url()."/m-subscription/?user_id=".$user_id."&user_name=".$user_name;
   	}
else{
 $go_url=pmpro_url("levels"); }
...
$_SESSION['log_href']="";
?>
<style>@media (min-width:769px){#pmpro_level-<?php echo $pmpro_level->id; ?>{max-width:50%;margin:0 auto;}}
...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<?php if($pmpro_review) { ?>
		<p><?php _e('Review the subscription information and pricing below then <strong>click the "Complete Payment" button</strong> to finish your order.', 'pmpro');?></p>
	<?php } ?>
	...
	<thead>
		<tr>
			<th>
				<span class="pmpro_thead-name"><?php _e('Subscription Plan Details', 'pmpro');?></span>
				<?php if(count($pmpro_levels) > 1) { ?><span class="pmpro_thead-msg"><a href="<?php echo $go_url ?>"><?php _e('change', 'pmpro');?></a></span><?php } ?>
			</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>
				<p>
					<?php printf(__('You have selected the <strong>%s</strong> subscription plan.', 'pmpro'), $pmpro_level->name); ?>
				</p>
...
//added
		...
		$delay_from_support=$wpdb->get_var("select delay from pmpro_dates_chk1 where user_id=".$this_user." order by id desc limit 1");
				...
				//echo "user_id:".$this_user."--usercount--".$userCount;
			
				if($userTotalCount >= 1)	{
					//echo "no-free trial";
					echo "<br><p>Free Trial/Delay awarded days:  ".(int)$delay_from_support."<sup>*</sup><br>";
					/*echo "<br><li>Free Trial/Delay awarded :  ".get_option("pmpro_sub_support_delay_".$this_user)."<sup>*</sup></li>";*/
					if((int)$delay_from_support == 0)
					echo "Paying Amount :  ".$pmpro_level->billing_amount."</p>";
					else
					echo "Paying Amount :  ".$pmpro_level->initial_payment."</p>";
				...
				if($pmpro_level->id != 4)
				echo "<br /><p>Free Trial awarded days:  ".(int)get_option("pmpro_subscription_delay_".$pmpro_level->id)."<br>";
...
?>
	...
	<?php } elseif($current_user->ID && !$pmpro_review) { ?>
		...
		<p id="pmpro_account_loggedin">
			<?php printf(__('You are logged in as <strong>%s</strong>. If you would like to use a different account for this subscription, <a href="%s">log out now</a>.', 'pmpro'), $current_user->user_login, wp_logout_url($_SERVER['REQUEST_URI'])); ?>
		</p>
	<?php } ?>
...
<?php global $current_user;
	...
	//echo "<script>console.log('".$this_user."')</script>";	
	//echo "<script>console.log('".$userCount."')</script>";	
	
$check=	($pmpro_level->id == 4) && ($userCount >= 1);	

//echo "<script>console.log('".$check."')</script>";
		
if( ($pmpro_level->id == 4) && ($userCount >= 1))
{
echo '<div align="center" style="width:50%;margin:0 auto !important">Your Free Trial already done. Please select <a style="color:blue !important;" href="'.site_url('subscription/').'">Another Plan</a></div>';

}
else {  ?>

	<div class="pmpro_submit" align="center" style="width:50%;margin:0 auto !important">
		<?php if($pmpro_review) { ?>

			<span id="pmpro_submit_span" style="float:unset !important">
...
<script>
var el = document.querySelector('input[type="image"]');
el.setAttribute("src","https://www.paypal.com/en_US/i/btn/btn_subscribe_LG.gif"); /* change image */
<!--
