...
<!-- <tbody> -->
	...
	<div  class="<?php if($count++ % 2 == 0) { ?>odd<?php } else { ?>even<?php } ?><?php if($current_level == $level) { ?> active<?php } ?>">
		...
		<p>
			...
			<p><?php if($level->id==1){echo "Save: 13.67%";} else if($level->id==2){echo "Save: 6.33%";} else if($level->id==3){echo "Basic Plan";} else echo "Special Offer";?></p>
				...
				<?php	if( ($level->id == 4) && ($userCount >= 1))
			{
			 echo '<a style="color: black !important;   background-color: white !important;" class="pmpro_btn disabled" id="<?php echo $level->id ?>" href="#">Free Trail Used</a>';
						 ...
						 <a class="pmpro_btn pmpro_btn-select" id="<?php echo $level->id ?>" href="<?php echo pmpro_url("checkout", "?level=" . $level->id, "https")?>"><?php _e('Select', 'pmpro');?></a>
			<?php } ?>


		<?php } elseif ( !$current_level ) { ?>                	
			<a  class="pmpro_btn pmpro_btn-select" plan="<?php echo $level->name ?>" onclick="checkActive(this.id,this.name)" id="<?php echo $level->id ?>" href="#" name="<?php echo pmpro_url("checkout", "?level=" . $level->id, "https")?> "><?php _e('Select', 'pmpro');?></a>
		<?php } elseif($current_level) { ?>      
			...
			<?php
				...
				if( pmpro_isLevelExpiringSoon( $current_user->membership_level) && $current_user->membership_level->allow_signups ) {
					?>
						<a class="pmpro_btn pmpro_btn-select" plan="<?php echo $level->name ?>" id="<?php echo $level->id ?>" onclick="checkActive(this.id,this.name)" href="#" name="<?php echo pmpro_url("checkout", "?level=" . $level->id, "https")?>"><?php _e('Renew', 'pmpro');?></a>
					<?php
				} else {
					?>	
						<a class="pmpro_btn disabled" plan="<?php echo $level->name ?>" id="<?php echo $level->id ?>" onclick="checkActive(this.id,this.name)" href="#" name="<?php echo pmpro_url("account")?>"><?php _e('Your&nbsp;Plan', 'pmpro');?></a>
...
<script>
function checkRedirect(id,name)
{
alert('ok');
}
</script>
...
if(id == <?php echo $current_user->membership_level->ID ?> )
{
...
swal({
    ...
    text: 'You are trying to renew your previously selected <strong><?php echo $current_user->membership_level->name ?> plan',
...
swal({
    title: 'Plan Change Info',
    text: 'You are trying to change your plan from  <strong><?php echo $current_user->membership_level->name ?></strong> plan to <strong> '+plan+' </strong> plan',
   confirmButtonText: 'Proceed',
    cancelButtonText: 'Cancel',
	showCancelButton: true,
    html: true,
