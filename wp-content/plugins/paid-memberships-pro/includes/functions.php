...
function pmpro_url($page = NULL, $querystring = "", $scheme = NULL)
{
	...
	//ios_app_user_agent_name
    	$site_url=site_url();
	$url=str_replace($site_url."/", $site_url."/m-", $url);}
	...
	/*if($_SESSION['app']==1){
		//app but not detected via agent/package so fetch it from session variable set
		$site_url=site_url();
		$url=str_replace($site_url."/", $site_url."/m-", $url);
...
function pmpro_getLevelCost(&$level, $tags = true, $short = false)
{
	//initial payment
	if(!$short)
		$r = sprintf(__('The amount for subscription is <strong>%s</strong> now', 'pmpro'), pmpro_formatPrice($level->initial_payment));
	else
...
$r .= sprintf(__(' <strong>%s every %d %s for %d more payments</strong>.', 'pmpro'), pmpro_formatPrice($level->billing_amount), $level->cycle_number, pmpro_translate_billing_period($level->cycle_period, $level->cycle_number), $level->billing_limit);
			}
		}
		...
		else
		{
			if( $level->billing_amount === $level->initial_payment ) {
				...
				{
					if(!$short)
						$r = sprintf(__('The amount for subscription is <strong>%s per %s</strong>.', 'pmpro'), pmpro_formatPrice($level->initial_payment), pmpro_translate_billing_period($level->cycle_period) );
					else
				...
				{
					if(!$short)
						$r = sprintf(__('The amount for subscription is <strong>%s every %d %s</strong>.', 'pmpro'), pmpro_formatPrice($level->initial_payment), $level->cycle_number, pmpro_translate_billing_period($level->cycle_period, $level->cycle_number) );
					else
...
$r=sprintf(__('<strong>%s USD every %d %s</strong>.', 'pmpro'), pmpro_formatPrice($level->billing_amount), $level->cycle_number, pmpro_translate_billing_period($level->cycle_period, $level->cycle_number));
	return $r;
}
...
function pmpro_getLevelExpiration(&$level)
{
	...
	{
		//$expiration_text = sprintf(__("Subscription expires after %d %s.", "pmpro"), $level->expiration_number, pmpro_translate_billing_period($level->expiration_period, $level->expiration_number));
		$expiration_text = "";
	}
	else
		$expiration_text = "";
...
function pmpro_changeMembershipLevel($level, $user_id = NULL, $old_level_status = 'inactive')
{
	...
	else
	{
		...
		if(empty($level_obj))
		{
			$pmpro_error = __("Invalid Plan.", "pmpro");
			return false;
		}
		$level = $level_obj->id;
	}
...
$txt="---------------- /n ";
//$old_planid=$wpdb->get_var("SELECT membership_id FROM wp_pmpro_memberships_users where user_id=".$user_id." and status<>'active' order by id desc limit 1"); //uncomment and delete "dbl_chk"
//dbl_chk start
...
$plan_period_no=$level['cycle_number'];
$plan_period=$level['cycle_period'];
...
//$trail_period_no=$level['expiration_number'];
//$trail_period=$level['expiration_period'];
...
if($allCount<2)
{
$delay= get_option("pmpro_subscription_delay_" . $level['membership_id'], "");
$txt.="delay:".$delay;
if($delay=="")
$free="free";
$abc="new user-".$free;
...
$wpdb->replace("pmpro_dates", array(
   "user_id" => $user_id,
...
mail("siddish.gollapelli@ideabytes.com","Inside Functions",print_r($txt,true));
...
function pmpro_toggleMembershipCategory( $level, $category, $value )
{
	...
	if ( ($level = intval($level)) <= 0 )
	{
		...
		{
			return __("Subscription Plan not found.", "pmpro");
		}
	}
...
function pmpro_checkDiscountCode($code, $level_id = NULL, $return_errors = false)
{
	...
	if(!$error)
	{
		...
		if(!empty($level_id) && $pmpro_check_discount_code_levels)
		{
			...
			if(empty($code_level))
				$error = __("This discount code does not apply to this Subscription Plan.", "pmpro");
		}
	}
