	...
	if ( $pmpro_continue_registration ) {
		...
		if ( $pmpro_msgt != "pmpro_error" ) {
			...
			if ( $pmpro_msgt != "pmpro_error" ) {
				...
				$txt="\n START:";	
				$txt.=$morder->membership_id."---".$morder->membership_name."--".$morder->InitialPayment."--";
			global $current_user;
			global $wpdb;
			get_currentuserinfo();
			$count=0;
		...
		$delay_from_support=$wpdb->get_var("select delay from pmpro_dates_chk1 where user_id=".$this_user." order by id desc limit 1");
				...
				$txt.="user_id:".$this_user."--usercount--".$userCount;
			
				//if($userCount >= 1)	{
				if($userTotalCount >= 1)	{
					$txt.="no-free trail";
					$morder->InitialPayment   = $pmpro_level->billing_amount; //not passing if no trail
				...
				//end
					if((int)$delay_from_support != 0)
					{
						$txt.="but delay";
						$morder->InitialPayment   = $pmpro_level->initial_payment;
						$txt.="delayed block-inital pay:".$morder->InitialPayment;
					}
					else 
					{
							$txt.="No delay";
				...
				else //new user $promotional_offer=true
				{

					
					$txt.="New User";
					if($promotional_offer)
					$morder->InitialPayment   = $pmpro_level->billing_amount;
			...
			//end
					...
					$txt.="DELAY:".$delay_from_support."active:".$another_plan_active ;

					if((int)$delay_from_support == 0 && (int)$another_plan_active > 0){
						$txt.="add plus one to effective date";
						$morder->ProfileStartDate = date( "Y-m-d", strtotime("+1 Days",current_time( "timestamp" ) )) . "T0:0:0";
					}else{
						$txt.="normal effective date";
					...
					if ( pmpro_isLevelTrial( $pmpro_level ) ) {
                                           if((int)$delay_from_support > 0 || (int)$userCount == 0){ 
						...
						$txt.="User-id:".$this_user."PaymentAmount:".$morder->PaymentAmount."--profile start date--".$morder->ProfileStartDate."--bill period--".$morder->BillingPeriod."--bill freq--".$morder->BillingFrequency;
						$txt.="trail period:".$morder->TrialBillingPeriod."-trailfreq-".$morder->TrialBillingFrequency."-trailcycle-".$morder->TrialBillingCycles."-amnt-".$morder->TrialAmount."-initial payment-".$morder->InitialPayment."-delay from support-".$delay_from_support."-usercount-".$userCount;

mail("siddish.gollapelli@ideabytes.com","Pre-Headers",print_r($txt,true));
