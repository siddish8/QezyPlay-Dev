...
if ( !class_exists('WordpressPopularPosts') ) {
	...
	class WordpressPopularPosts extends WP_Widget {
		...
		protected $defaults = array(
			...
			'markup' => array(
				'custom_html' => false,
				'wpp-start' => '&lt;ul class="wpp-list" style="display:inline-flex" &gt;',
				'wpp-end' => '&lt;/ul&gt;',
		...
		public function widget( $args, $instance ) {
			...
			echo $after_widget . "\n";
			$swal="";
			$swal.="<script>$('ul.wpp-list > li > a').click( function(e) {e.preventDefault(); 
var href = jQuery(this).attr('href');";
 if(!is_user_logged_in()) { 
	$swal.="swal('Please Log-In to view this channel');";
 }else { 
	$swal.="window.location.href=href;";
 } 

$swal.=" return false; } );</script>"."\n";
echo $swal;
		} // end widget
		...
		public function update( $new_instance, $old_instance ) {
			...
			$instance['markup']['wpp-start'] = empty($new_instance['wpp-start'])
			  ? htmlspecialchars( '<ul class="wpp-list" style="display:inline-flex">', ENT_QUOTES )
		...
		private function __get_popular_posts( $instance ) {
			...
			if ( !is_array($mostpopular) || empty($mostpopular) ) {
				return apply_filters( 'wpp_no_data', "<p class=\"wpp-no-data\">" . __('Loading...', $this->plugin_slug) . "</p>" );
						}
			...
			if ($instance['markup']['custom_html']) {
				...
				$content .= "\n" . "<ul class=\"wpp-list\" style=\"display:inline-flex;\">" . "\n";
			}
		...
		private function __render_popular_post($p, $instance) {
			...
			if ($instance['markup']['custom_html']) {
				...
				$data = array(
					'title' => '<a style="display:none;" href="'.$permalink.'" title="'. esc_attr($title) .'" class="wpp-post-title" target="' . $this->user_settings['tools']['link']['target'] . '">'.$title_sub.'</a>',
					'summary' => $excerpt,
					'stats' => $_stats,
			...
			// build regular layout
				...
				$thumb = ( !empty($thumb) ) 
				  ? '<a ' . ( ( $this->current_post_id == $p->id ) ? '' : 'href="' . $permalink . '"' ) . ' title="' . esc_attr($title) . '" target="' . $this->user_settings['tools']['link']['target'] . '" class="wpp-th-link">' . $thumb . '</a> '
				  : '';
		 ...
		 * @return	string
		 */
		private function __get_img($p, $id = null, $url = null, $dim = array(80, 80), $crop = false, $source = "featured", $title) {
		...
		public function shortcode($atts = null, $content = null) {
			...
			*/
			...
			$shortcode_ops = array(
				...
				'markup' => array(
					'custom_html' => true,
					'wpp-start' => empty($wpp_start) ? '<ul class="wpp-list" style="display:inline-flex">' : $wpp_start,
