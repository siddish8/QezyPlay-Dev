...
function sub_pagination($count2){
	...
	$actual_link="?";	
	 	foreach($_GET as $key => $value){
	 		if($key != 'page_number')$actual_link.="{$key}=$value&amp;"; 		
...
function html_showhugeit_contacts( $rows,  $pageNav,$sort,$cat_row,$form_styles){
	global $wpdb;
	?>
...
function html_view_submissions($submitionsArray, $submitionsCount,$limitPage,$subName,$id) {
$keyForBackground = 1;
?>
<div class="wrap">
	<?php drawFreeBanner();?>
