...
/*Process uploads*/
	...
	public function confirm_reset_password()
	{
		...
		if($validation=="" )
		{
			...
			if($user->ID >0 )
			{
				...
				$html .= "<div class=''>".__('<a href="'.site_url().'/login" title="Login">CLICK HERE TO LOGIN</a>', 'xoousers')."</div>";
	...
	public function send_reset_link()
	{
		...
		if(is_email($_POST['user_login']))
		{
			  ...
			  {
				  //user not found
				  $html = __('<span class="alert alert-danger">Email not found</span>','xoousers');
				  ...
				  //user found 				  
				   ...
				   {					   					   
					   //user is not active					   
					   $html = __('<span class="alert alert-danger">Your account is not active yet.</span>','xoousers');				   
					   $noactive = true;
			  ...
			  {
				  //user not found
				  $html = __('<span class="alert alert-danger">User not found</span>','xoousers');
	...
	public function edit_profile_form( $sidebar_class=null, $redirect_to=null )
	{
		...
		$html .= '<form action="" method="post" id="xoouserultra-profile-edition-form" enctype="multipart/form-data">';
			...
			if ( $type == 'usermeta' && $deleted == 0 && $private == 0)			
			 {
				...
				$html .= '<div class="xoouserultra-field-value">';
					...
					{
					$html.='<input type="text" onclick="return styleback(this.id)" maxlength="10" class="xoouserultra-input'.$required_class.'" 					name="'.$meta.'" id="'.$meta.'" value="'.$this->get_user_meta( $meta).'"  title="'.$name.'" 						'.$disabled.' /></div></div><div class="xoouserultra-clear"></div>';
					continue;}
					if($meta=='phone')
					{
					$html.='<input type="text" class="xoouserultra-input'.$required_class.'" 					name="'.$meta.'" id="'.$meta.'" value="'.$this->get_user_meta( $meta).'"  title="'.$name.'" onblur="return validate(this.id)" onclick="return styleback(this.id)" /></div><div id="phone_err_div" class="xoouserultra-field" style="display:none">
    <label id="phone_label" class="xoouserultra-field-type"></label><div id="phone_err" class="alert alert-danger xoouserultra-field-value"></div>
   </div></div><div class="xoouserultra-clear"></div>';
					continue;}
				...
				if ($can_edit == 0)
				{
					...
					switch($field) {
case 'fileupload':
							$html.='<input style="display:block !important;" type="file" onclick="return styleback(this.id)" class="xoouserultra-input'. $required_class.'" name="'.$meta.'" id="'.$meta.'" value="'.$this->get_user_meta( $meta).'" accept="image/*" title="'.$name.'" '.$disabled.' ><div id="pic_err_div" class="xoouserultra-field" style="display:none">
    <label id="phone_label" class="xoouserultra-field-type"></label><div id="pic_err" class="alert alert-danger xoouserultra-field-value"></div>
   </div>';
							break;
					
						case 'textarea':
							$html .= '<textarea onclick="return styleback(this.id)" class="xoouserultra-input'.$required_class.'" name="'.$meta.'" id="'.$meta.'" title="'.$name.'" '.$disabled.'>'.$this->get_user_meta( $meta).'</textarea>';
							break;
							
						case 'text':
							$html .= '<input onclick="return styleback(this.id)" type="text" class="xoouserultra-input'.$required_class.'" name="'.$meta.'" id="'.$meta.'" value="'.$this->get_user_meta( $meta).'"  title="'.$name.'" '.$disabled.' />';
							break;
							
							
						case 'datetime':
						    $html .= '<input onclick="return styleback(this.id)" type="text" class="xoouserultra-input'.$required_class.' xoouserultra-datepicker" name="'.$meta.'" id="'.$meta.'" value="'.$this->get_user_meta( $meta).'"  title="'.$name.'" '.$disabled.' />';
						    break;
						...
						case 'select':
							...
							if (isset($loop)) 
							{
								$html .= '<select onclick="return styleback(this.id)" class="xoouserultra-input'.$required_class.'" name="'.$meta.'" id="'.$meta.'" title="'.$name.'" '.$disabled.'>';							
...
<input type="submit" onclick="return validate(this.id)" name="xoouserultra-update" id="xoouserultra-update" class="xoouserultra-button" value="'.__('Update','xoousers').'" />
						</div>
	...
	function update_me() 
	{
		global  $xoouserultra;global $errMsg;
		...
		$file22=fopen("edit_prof_chk","a");
		fwrite($file22,"Start:");
		//echo "<pre>";
		foreach ($array as $arr) {
			if($arr['meta']!= "")
			     	$newArray[$arr['meta']] = $arr['name'];
		}
		...
		$_SESSION['errMsg'] = "";
			...
			extract($field);
           		 ...
           		 if (isset($array[$key]['field']) && $array[$key]['field'] == 'fileupload') 
				{
				$info="key:".$key;
				fwrite($file22,$info);
				$current_user = wp_get_current_user();
				$user_id=$current_user->ID;
				//$info="img-url:".get_avatar_url( $user_id );
				//fwrite($file22,$info);
				global $wpdb;
				$info=$wpdb->get_var("SELECT meta_value from wp_usermeta where user_id=".$user_id." and meta_key='user_pic' ");fwrite($file22,$info);
				if($_FILES["user_pic"]["name"]!=""){
				
					$files = $_FILES['user_pic']['name'];
					$info=$files;					
					fwrite($file22,$info);
					$file=$_POST[$meta];
					
					$info="form value:".$file;
					fwrite($file22,$info);
					
					$site_url=site_url().'/';
					$upload_folder = $xoouserultra->get_option('media_uploading_folder');
					$path = $site_url."wp-content/uploads/".$upload_folder."/".$user_id."/";
					//echo $path;
				

					$image_extention = explode("/",$_FILES["user_pic"]["type"]);
			       		 $imageFileType = $image_extention[1];
					//$imageFileType=pathinfo($files,PATHINFO_EXTENSION);

					if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"
				&& $imageFileType != "gif" ) 
					{
				    		$_SESSION['errMsg'] =  "Error: Choose only Image(JPG, JPEG, PNG & GIF) files for Profile Picture | <style>#user_pic{border: 1px solid red ;
    box-shadow: 0 0 10px red ;
    outline: none;}</style><script>document.getElementById('pic_err_div').style.display='block';document.getElementById('pic_err').innerHTML='Choose only Image(JPG, JPEG, PNG & GIF) files for Profile Picture';</script>";
					}
					else{
					
						//$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/newqezyplay/wp-content/uploads/'.$upload_folder."/".$user_id."/";
						$uploaddir = ABSPATH.'wp-content/uploads/'.$upload_folder."/".$user_id."/";
				
						//$uploaddir = site_url().'/wp-content/uploads/'.$upload_folder."/".$user_id."/";
						//echo $uploaddir;
						...
						if (move_uploaded_file($_FILES['user_pic']['tmp_name'], $uploadfile)) {
							...
							$i = ob_get_clean();
							    // Save file
							$newfilepath=$uploaddir."resized-".$files;
							$newfiles="resized-".$files;
							$fp = fopen ($uploaddir."resized-".$files,'w');
							fwrite ($fp, $i);
							fclose ($fp);
					  		 ...
					  		 $_SESSION['errMsg'] = "<strong>Error:</strong> ProfilePicture Upload failed | <style>#user_pic{border: 1px solid red ;
    box-shadow: 0 0 10px red ;
    outline: none;}</style><script>document.getElementById('pic_err_div').style.display='block';document.getElementById('pic_err').innerHTML='ProfilePicture Upload failed';</script>";
						}			
				
					//update_user_meta($user_id, 'user_pic', $files);
						if($count>0){	
							update_user_meta($user_id, 'user_pic', $newfiles);
							$_SESSION['pic']="Profile Piture";
							$info="COUNT:".$count;
							fwrite($file22,$info);
						   }
						else{
			...
			if (!isset($this->errors)) 
			{
				...
				foreach($this->usermeta as $key => $value) 
				{
					...
					if($key=="phone")
					{
							...
							if($value=="")
							{
								 $_SESSION['errMsg'].='<strong>ERROR:</strong> Please enter a contact no. | <style>#phone{border: 1px solid red ;
    box-shadow: 0 0 10px red ;
    outline: none;}</style><script>document.getElementById("phone_err_div").style.display="block";document.getElementById("phone_err").innerHTML="Please enter a contact no.";</script>';}
							elseif(!preg_match("/^\+(?:[0-9] ?){6,14}[0-9]$/", $value))
							{
					
								$_SESSION['errMsg'] .='<strong>ERROR:</strong> Please enter a valid contact no. | <style>#phone{border: 1px solid red ;
    box-shadow: 0 0 10px red ;
    outline: none;}</style><script>document.getElementById("phone_err_div").style.display="block !important";document.getElementById("phone_err").innerHTML="Please enter a valid contact no.";</script>'; 
							}
							else
							{
								...
								if($value!=$old_value)
								//$upd.="|".$newArray[$key]; //send update msg only when	
								$upd.=$newArray[$key]."|";

							}
					}
					
					if($value!=$old_value and $key!="phone")
						//$upd.="|".$newArray[$key]; //send update msg only when	
						{$upd.=$newArray[$key]."|";}
				...
				//upate activity
					...
					if($upd!=""){
					$_SESSION["update_msg"]="UPDATED : ".$upd;}								
			}//no error end
	...
	******************************************/
	public function show_users_directory($atts)
	{
		...
		if($display_to_logged_in_only=='yes' && !is_user_logged_in())
		{
			...
			//$html .='<ul class="usersultra-front-results"  display="inline">';
			...
			if(count($users_list['users'])>0)
			{
				foreach($users_list['users'] as $user)
				{
				   ...
				   if($pic_boder_type=="rounded")
				   {
					...
					$html .=' </div> ';
					 ...
					 //$html .='<div class="xoousers-prof-photo" align="left" position="absolute" style="margin-left=0; margin-top=0">';
					   
					$html.='<div class="full" style="border:0.25px hidden black">';   
				...
				//$html .='<div class="info-div">';
						...
						$html .= '<div class="logo"><p class="logo" style="position:absolute;margin-bottom:10px; margin-left:10px; " >'.$xoouserultra->userpanel->get_user_pic( $user_id, $pic_size, $pic_type, $pic_boder_type, $pic_size_type).'</p>';             
					
					$html .='</div>';
		
						$html .=' <div class="alltext" style="margin-left:200px;margin-top:15px" ><h2 class="uu-direct-name" style="color:red ;"<strong><a style="color:indigo" class="uultra-btn-profile" href="?page_id=268/'. $user_id.'/">'.  $xoouserultra->userpanel->get_display_name($user_id).'</a></strong></h2>';
						...
						$html.='  <div class="uultra-view-profile-bar"/>';
						 //$html .='</div> 
						 
						 // <div class="uultra-view-profile-bar" >';
						  
						 // $html .=' <p> <a class="uultra-btn-profile" href="'.$xoouserultra->userpanel->get_user_profile_permalink( $user_id).'">'.__("View Details","xoousers").'</a></p></p>
						  $html.='<p><a class="uultra-btn-profile" href="?page_id=268/'. $user_id.'/" >View Details</a></p></p>
						   ';
					$html.='</div></div>';$html .=' <div style="width:100%" class="social-icon-divider" >                                       
 ...
 $html .='</div>';
	...
	public function get_total_found($users_list)
	{
		...
		$html= '<div class="uultra-search-results">
			<h1><u>'.__('Registered Users: ','xoousers').''.$total .' '.$text.'./*$text.*/</u></h1></div>';
	...
	public  function display_optional_fields ($user_id, $display_country_flag, $fields_to_display) 
	{
		...
		
		{
			...
			foreach($array as $key => $fieldP)
				{
					if( $fieldP['meta']==$field)
					{
						extract($fieldP);
						...
						{				
							...
							if (isset($array[$key]['name']) && $name)
							 {
								//echo $name.$fieldP['meta']."<br>";
							}
				
						}
					
					  }
			        }//end_foreach($array.......$fieldP)
				...
				if( $field =='country')
				{
					...
					if($u_meta=="")				
					{
						$u_meta = __("<span class='data-b'>Not yet set</span>", 'xoousers');
					
					
					}
					
					$fields_list .= "<p class='desc' style='margin-left:70px'><span class='data-a'>Our Corporate description </span> : <span class='data-b'> <strong> ".$u_meta."</strong></span><br></p>";
					...
					$fields_list .= "<p style='margin-left:70px'><span class='data-a'>".$name."</span><span class='data-b'><strong>".$u_meta."</strong><br></span></p>";
...
?>
	...
	******************************************/
	...
	public  function display_optional_fields2 ($user_id, $display_country_flag, $fields_to_display) //members directory for IoT 
	{
		 global  $xoouserultra;
		...
		
		{
			...
			foreach ($fields as $field) 
			{
				...
				if( $field =='country')
				{
					...
					if($u_meta=="")				
					{
						$u_meta = __("This user has not set this yet", 'xoousers');
	...
	public function  display_optional_fields3($user_id, $display_country_flag, $fields_to_display)	
	{
		if($fields_to_display=='all')
		{
		$array = get_option('usersultra_profile_fields');
		...
		foreach($array as $key => $field) 
		{
			...
			//if ( $type == 'usermeta' && $deleted == 0 && $private == 0  && isset($array[$key]['show_in_register']) && $array[$key]['show_in_register'] == 1) && $can_hide ==0)	//changed on 12-07-2016	
if ( $type == 'usermeta' && $deleted == 0 && $private == 0  && isset($array[$key]['show_in_register']) && $can_hide ==0)	
			{				
				/* Show the label */
				if (isset($array[$key]['name']) && $name)
				{
					if($name=="Profile picture"); else $html .= ' <span class="data-a">'.$name.'</span><span class="data-b"><strong>'.$this->get_custom_user_meta( $meta, $user_id).'</strong></span> ';
				}
		...
		$html='';
		$html.='<h2>Set the options_to_display="all" in edit page for full profile </h2>';
	}
}
public function  get_profile_info2 ($user_id)	
	{
		...
		foreach($array as $key => $field) 
		{
			...
			{				
				...
				if (isset($array[$key]['name']) && $name)
				{
					if($meta=="user_pic"); else $html .= ' <span class="data-a">'.$name.'</span><span class="data-b"><strong>'.$this->get_custom_user_meta( $meta, $user_id).'</strong></span> ';
				}
...
public function  get_profile_info ($user_id)	
	{
		...
		foreach($array as $key => $field) 
		{
			...
			{				
				...
				if (isset($array[$key]['name']) && $name)
				{
					if($meta=="user_pic"); else$html .= ' <span class="data-a">'.$name.':</span><span class="data-b">'.$this->get_custom_user_meta( $meta, $user_id).'</span> ';
				}
