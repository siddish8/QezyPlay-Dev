...
class XooUserAdmin extends XooUserUltraCommon 
{
	...
	function userultra_set_option($option, $newvalue)
	{
		...
		$settings[$option] = sanitize_text_field($newvalue);
	...
	public function set_default_email_messages()
	{
		...
		$email_body .= __("Thanks for registering. Your account is now active. <br />","xoousers") .  $line_break.$line_break;
	...
	function admin_page() 
	{
		...
		if (isset($_POST['update_settings'])) {
			...
			if ( ! isset( $_POST['uultra_nonce_check'] ) || ! wp_verify_nonce( $_POST['uultra_nonce_check'], 'update_settings' ) 
			) 
			{
