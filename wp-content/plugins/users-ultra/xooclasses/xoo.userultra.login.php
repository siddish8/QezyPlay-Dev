				...
				if(!$noactive)
				{
					...
					if ( is_wp_error($user) ) 
					{
						...
						if ($user->get_error_code() == 'invalid_username') {
							...
							$this->errors[] = __('<strong>ERROR:</strong> Invalid credentials entered.','xoousers');
						}
					...
					//not active
					$this->errors[] = __('<strong>ERROR:</strong> Your account is not active. Click on activation link from your mail','xoousers');
...
//added
function new_user_regsub($user_id,$startdate)
{
global $wpdb;
$txt="---------------- /n ";
...
$txt.="startdate:".$startdate."-usercount-".$userCount."-delay-".$delay."-nextpay-".$next_paydate;
			...
			$fileR=fopen("registrations.txt","a");
			 ...
			 //check if already registered
			  ...
			  if(!$exists)
			  {
				 ...
				 //generat random password
				// $user_pass = wp_generate_password( 12, false); //change this to username
				...
				if ( ! $user_id ) 
				{
						...
						//added for fb on 10-08-2016
						$fb_user_url='https://www.facebook.com/app_scoped_user_id/'.$u_fb_id.'/';
						global $wpdb;
						$wpdb->update("wp_users", array(   
					"user_url" => $fb_user_url), array("ID"=>$user_id)
					);	

						//added for subscription after registration 

		
						//Registration Done: Give 7-day free Trail:
						...
						$this->new_user_regsub($user_id,$startdate);
