	...
	public function xoousers_create_standard_fields ()	
	{
		...
		if ($set_pass) 
		{
			...
			200 => array( 
				...
				'required' => 1,
				'can_hide' => 0,
				'help' => __('Re-enter your password','xoousers')
			),
			250 => array(
				'icon' => 0,
...
$date_picker_array = array(
		            'closeText' =>  __('Done',"xoousers"),
		            'prevText' =>  __('Prev',"xoousers"),
		            'nextText' => __('Next',"xoousers"),
		            'currentText' => __('Today',"xoousers"),
		            'monthNames' => array(
		                        'Jan' =>  __('January',"xoousers"),
    		                    'Feb' =>  __('February',"xoousers"),
    		                    'Mar' =>  __('March',"xoousers"),
    		                    'Apr' =>  __('April',"xoousers"),
    		                    'May' =>  __('May',"xoousers"),
    		                    'Jun' =>  __('June',"xoousers"),
    		                    'Jul' =>  __('July',"xoousers"),
    		                    'Aug' =>  __('August',"xoousers"),
    		                    'Sep' =>  __('September',"xoousers"),
    		                    'Oct' => __('October' ,"xoousers"),
    		                    'Nov' =>  __('November' ,"xoousers"),
    		                    'Dec' =>  __('December' ,"xoousers")
		                    ),
		            ...
		            'dayNames' => array(
		                        'Sun' => __('Sunday'  ,"xoousers"),
    		                    'Mon' =>  __('Monday'  ,"xoousers"),
    		                    'Tue' => __( 'Tuesday'  ,"xoousers"),
    		                    'Wed' =>  __( 'Wednesday'  ,"xoousers"),
    		                    'Thu' =>  __(  'Thursday'  ,"xoousers"),
    		                    'Fri' =>   __('Friday'  ,"xoousers"),
    		                    'Sat' =>  __('Saturday'  ,"xoousers")
		                    ),
		            'dayNamesShort' => array(
		                        'Sun' => __('Sun'  ,"xoousers") ,
    		                    'Mon' => __('Mon'  ,"xoousers"),
		...
		wp_localize_script('uultra-front_js', 'XOOUSERULTRADATEPICKER', $date_picker_array);
	...
	public function password_reset( $args=array() ) {
		...
		//style="background: url('.site_url().'/wp-content/uploads/2016/09/bck2.jpg)"
		$display = null;
		...
		$display .= '<div class="xoouserultra-head" style="background:none;padding:0px 20px">';
		    $display .='<div class="xoouserultra-left">';
		        $display .='<div style="margin: 15px auto;" class="xoouserultra-field-name xoouserultra-field-name-wide login-heading" id="login-heading-'.$this->login_code_count.'">'.__('Password Reset','xoousers').'</div>';
		    $display .='</div>';
	...
	public function show_password_reset_form( $sidebar_class=null, $args, $key) 
	{
		...
		$display .= '<input onblur="validate(this.id)" onclick="styleback(this.id)" type="password" class="xoouserultra-input" name="'.$meta.'" id="'.$meta.'" value="" '.$placeholder.' />';		
		$display .= '</div><div id="'.$meta.'_err_div" class="xoouserultra-field" style="display:none">
    <label id="'.$meta2.'_label" class="xoouserultra-field-type"></label><div id="'.$meta.'_err" class="alert alert-danger xoouserultra-field-value"></div>
   </div>';		
		...
		$display .= '<div class="xoouserultra-field-value">';		
		$display .= '<input onblur="validate(this.id)" onclick="styleback(this.id)" type="password" class="xoouserultra-input" name="'.$meta_2.'" id="'.$meta_2.'" value="" '.$placeholder.' />';		
		$display .= '</div><div id="'.$meta2.'_err_div" class="xoouserultra-field" style="display:none">
    <label id="'.$meta2.'_label" class="xoouserultra-field-type"></label><div id="'.$meta2.'_err" class="alert alert-danger xoouserultra-field-value"></div>
   </div>';		
		$display .= '</div><div class="xoouserultra-clear"></div>';
		...
		$display .= '<input style="margin: 20px auto;" type="submit" onclick="return validate(this.id)" name="xoouserultra-login" class="btn btn-success'.$login_btn_class.'" value="'.__('Reset Password','xoousers').'" id="xoouserultra-reset-confirm-pass-btn" />';
	...
	$("#preset_password").click(function() {
 $(".xoouserultra-errors").hide();
$(".uupublic-ultra-success").hide();
});
$(".blog-heading h1").hide()</script>';		
	...
	public function login( $args=array() ) {
		...
		$display = null;
		$display .= '<div align="center" class="xoouserultra-wrap xoouserultra-login '.$sidebar_class.'">
					<div class="xoouserultra-inner xoouserultra-login-wrapper">';
		//style="background: url('.site_url().'/wp-content/uploads/2016/09/bck2.jpg)"
		$display .= '<div class="xoouserultra-head" style="background:none;padding:0px 20px">';
		    $display .='<div class="xoouserultra-left">';
	...
	public function show_login_form( $sidebar_class=null, $redirect_to=null, $args) 
	{
		...
		$display .= $this->get_social_buttons(__("Login with",'xoousers' ),$args);
...
$display .= '<label class="xoouserultra-field-type" for="'.$meta.'">'; 
$display .= '<i class="fa fa-'.$icon.'"></i>';
 $display .= ''.$name. ' '.$required_text.'</label>';
							$display .= '<input onkeypress="return event.charCode !=32" onblur="validate(this.id)" onclick="styleback(this.id)" required="required" type="text" class="xoouserultra-input'.$input_ele_class.'" name="'.$meta.'" id="'.$meta.'" value="'.$this->get_post_value($meta).'" '.$placeholder.' />';
...
$display .= '<i class="fa fa-'.$icon.'"></i>';
 $display .= ''.$name. ' '.$required_text.'</label>';
							$display .= '<input onkeypress="return event.charCode !=32" onblur="validate(this.id)" onclick="styleback(this.id)" required="required" type="password" class="xoouserultra-input'.$input_ele_class.'" name="'.$meta.'" id="'.$meta.'" value="" '.$placeholder.' />';
							break;
					}
				...
				$display .= '</div>';
				$display .= '<div id="'.$meta.'_err_div" class="xoouserultra-field" style="display:none">
    <label id="'.$meta.'_label" class="xoouserultra-field-type"></label><div id="'.$meta.'_err" class="alert alert-danger xoouserultra-field-value"></div>
   </div>';
		...
		$forgot_pass = '<a style="color: #FF5722;" href="../forgot-password/" id="xoouserultra-forgot-pass-'.$this->login_code_count.'" class="xoouserultra-login-forgot-link" title="'.__('Forgot Password?','xoousers').'">'.__('Forgot Password?','xoousers').'</a>';
		...
		{			
			$register_link_url = '<br /><a style="background-color:#3994b7;color:white;border:1px solid #3994b7;margin: 8px auto;" href="'.$register_link.'" class="btn btn-default">'.__('New Registration','xoousers').'</a>';//xoouserultra-login-register-link
		}
		
		if($disable_registration_link!='yes' && $custom_registration_link!='')
		{			
			$register_link_url = ' <br /> <a href="'.$custom_registration_link.'" class="btn btn-default">'.__('Register','xoousers').'</a>';//xoouserultra-login-register-link
		}
		...
		<input type="checkbox" name="rememberme" id="rememberme_'.$this->login_code_count.'" value="0" /> <label style="margin-bottom:20px;float:left;margin-top: 10px;" for="rememberme_'.$this->login_code_count.'"><span></span>'.__('Remember me','xoousers').'</label>
		
		</div>
		<div><p style="color:black;margin:0;font-size:10px;float:right" id="tos_pp">*By clicking on Log In, you agree to our <a href="../terms-of-service">Terms of Service</a> and that you have read our <a href="../privacy-policy">Privacy Policy</a></p> </div>
		<input onclick="return validate(this.id)" type="submit" id="Login"  name="xoouserultra-login" class="btn btn-success'.$login_btn_class.'" value="'.__('Log In','xoousers').'" /><br /><div align="center">'.$forgot_pass.$register_link_url;
...
$("#user_login").click(function() {
 ...
 $(".blog-heading h1").hide();

function hide()
{
document.getElementById("xoouserultra-forgot-pass-holder").style.display="none";
}
</script>';
...
/* forgot paswd form */ //added on 01-07-2016
...
$forgot_pass = '';
		...
		$forgot_pass .= '<div class="xoouserultra-inner xoouserultra-login-wrapper" style="padding: 10px;
    max-width: 500px;
    margin: 0 auto;
    border: 1px solid #ddd;
    box-shadow: 0 1px 2px -1px #ccc;
    background-color: #fff;
    border-radius: 3px;background: url('.site_url().'/wp-content/uploads/2016/09/bck2.jpg)">
<div class="xoouserultra-forgot-pass" style="display:block !important" id="xoouserultra-forgot-pass-holder">';
		
		
		$forgot_pass .= "<div class='notimessage'>";
		
		$forgot_pass .= "<div class='uupublic-ultra-info'>".__(" A quick access link will be sent to your email that will let you get in your account and change your password. ", 'xoousers')."</div>";
		
		$forgot_pass .= "</div>";
		
		
		$forgot_pass .= '<form> <div class="xoouserultra-field xoouserultra-edit xoouserultra-edit-show">';
		...
		$forgot_pass .= '<input type="text" required="required" onkeypress="return event.charCode !=32" onblur="validate(this.id)"  onclick="styleback(this.id)" type="text" class="xoouserultra-input" name="user_name_email" id="user_name_email" value=""></div>';
		$forgot_pass .= '</div>';
		    
			//$forgot_pass .= '<div id="err_div" style="display:none" class="xoouserultra-field"><label id="user_name_email_label" class="xoouserultra-field-type"></label><div id="user_name_email_err" class="xoouserultra-errors xoouserultra-field-value"></div></div>';
$forgot_pass .= '<div id="user_name_email_err_div" class="xoouserultra-field" style="display:none">
    <label id="user_name_email_label" class="xoouserultra-field-type"></label><div id="user_name_email_err" class="alert alert-danger xoouserultra-field-value"></div>
   </div>';
		...
		$forgot_pass.='<div class="xoouserultra-field-value">';
		           ...
		           $forgot_pass.='<input onclick="return validate(this.id);" type="submit" name="xoouserultra-forgot-pass" id="xoouserultra-forgot-pass-btn-confirm" class="btn btn-success" value="'.__('Send me Password','xoousers').'">';
					
$forgot_pass.='<div class="xoouserultra-back-to-login">';
		$forgot_pass.='<a style="color: #FF5722;" href="../login/" onclick="hide()"  title="'.__('Back to Login','xoousers').'" id="xoouserultra-back-to-login-'.$this->login_code_count.'">'.__('Back to Login','xoousers').'</a><br /> ';
		
		$forgot_pass.='</div>';
					$forgot_pass.='<div class="xoouserultra-signin-noti-block xoouserultra-field-value" id="uultra-signin-ajax-noti-box"> ';
		
		$forgot_pass.='</div>';
		            
		        $forgot_pass.='</div></form>';
...
<script>
$(".blog-heading h1").hide();
if($.trim($("#uultra-signin-ajax-noti-box").html())=="")
{
$("#uultra-signin-ajax-noti-box").hide();
$("#uultra-signin-ajax-noti-box").css({"border":"none"});
}</script>';	
...
return $forgot_pass;
	...
	function show_registration_form( $args=array() )
	{
		...
		$defaults = array(
        'use_in_sidebar' => null,
		'form_header_text' => __('Register here for Free Channels and 1-Week Access to Premium Channels*','xoousers'),
		'custom_text' => '',
        'redirect_to' => null
...
//style="background: url('.site_url().'/wp-content/uploads/2016/09/bck2.jpg)"
		    ...
		    $display .= '<script>var clear;</script><div align="center" class="xoouserultra-wrap xoouserultra-registration '.$sidebar_class.'">
					<div class="xoouserultra-inner">
					
						<div class="xoouserultra-head" style="background:none;padding:0px 20px">
					...
					//get social sign up methods
		$display .= $this->get_social_buttons(__("Register with",'xoousers' ),$args);
	...
	function display_the_registeration_form( $sidebar_class=null, $redirect_to=null , $args)
	{
		...
		$display .= '<div class="xoouserultra-seperator-requiredfields xoouserultra-edit xoouserultra-edit-show">'.__('Fields with (<span style="color:red">*</span>) are required','xoousers').'</div>';	
		...
		foreach($this->registration_fields as $key=>$field) 
		{
			...
			if ( $type == 'usermeta') {
				...
				if($required == 1 && in_array($field, $this->include_for_validation))
				{
					...
					$required_class = '';
					//$required_text = '(*)';
					$required_text = '<span style="float:right;color:red"><sup>*</sup></span>';
				}
				...
				$display .= '<div class="xoouserultra-field-value">';
					...
					switch($field) {
						...
						case 'textarea':
							$display .= '<textarea style="resize:none"  placeholder="'.$name.'" class="'.$required_class.' xoouserultra-input" name="'.$meta.'" id="reg_'.$meta.'" title="'.$name.'" data-errormessage-value-missing="'.__(' * This input is required!','xoousers').'">'.$this->get_post_value($meta).'</textarea>';
							break;
						
						case 'text':
							$display .= '<input onkeyup="return avoidspace(this.id)" onkeydown="return event.charCode !=32" onkeypress="return event.charCode !=32" onblur="validate(this.id)"  onclick="styleback(this.id)" type="text" class="'.$required_class.' xoouserultra-input" name="'.$meta.'" id="reg_'.$meta.'" value="'.$this->get_post_value($meta).'" title="" data-errormessage-value-missing="'.__(' * This input is required!','xoousers').'"/>';
							
							if (isset($this->registration_fields[$key]['help']) && $help != '') {
								$display .= '<div class="xoouserultra-help"><b>'.$help.'</b></div><div class="xoouserultra-clear"></div>';
							}
		//UserName should not contain special char and shouldn't start with underscore and must be minm. of length 4
								if($meta=="user_login"){$help="User name must be between 4 - 15 alphanumeric characters(Aa-Zz 0..9) and should not start with underscore";}
								if($meta=="user_email"){$help="Please check your email – you will have to respond to a validation email to get the free trial";}
								
								$display .= '<div id="reg_'.$meta.'-help" class="xoouserultra-help">'.$help.'</div><div class="xoouserultra-clear"></div>';
						...
						case 'password':
							...
							$display .= '<input onkeypress="return event.charCode !=32" onblur="return validate(this.id)" onclick="styleback(this.id)" type="password" class="'.$required_class.' xoouserultra-input password" name="'.$meta.'" id="reg_'.$meta.'" value="" autocomplete="off" title="" data-errormessage-value-missing="'.__(' * This input is required!','xoousers').'" />';
							
							if (isset($this->registration_fields[$key]['help']) && $help != '') {
//Password must be of 4 minimum length
								if($meta=="user_pass")$help="Password must be minimum 8-16 characters";
								$display .= '<div id="reg_'.$meta.'-help" class="xoouserultra-help">'.$help.'</div><div class="xoouserultra-clear"></div>';
							}
						...
						case 'password_indicator':
							$display .= '<div class="password-meter" style="display:none"><div class="password-meter-message" id="password-meter-message">&nbsp;</div></div>';
							break;
				...
				$display .= '<div id="reg_'.$meta.'_err_div" class="xoouserultra-field" style="display:none">
    <label id="'.$meta.'_label" class="xoouserultra-field-type"></label><div id="reg_'.$meta.'_err" class="alert alert-danger xoouserultra-field-value"></div>
   </div>';
				...
				if($meta=='user_email' && $password_retype!='no')
				{
					...
					$display .= '<input type="text" class="'.$required_class.' xoouserultra-input" name="user_email_2" placeholder="'.__('Re-type your email', 'xoousers').'" id="reg_user_email_2" value="'.$this->get_post_value('user_email_2').'" title="Re-type your email." data-errormessage-value-missing="'.__(' * This input is required!','xoousers').'"/>';
			...
			{				
				...
				$display .= '<div class="xoouserultra-field-value">';
					...
					switch($field) {
						...
						case 'textarea':
							$display .= '<textarea placeholder="'.$name.'" class="'.$required_class.' xoouserultra-input" name="'.$meta.'" id="'.$meta.'" title="'.$name.'" data-errormessage-value-missing="'.__(' * This input is required!','xoousers').'">'.$this->get_post_value($meta).'</textarea>';
							break;
						...
						case 'text':
							...
							if($meta=="phone")
							{
							$display .= '<input required="required" onkeypress="return event.charCode !=32" onblur="validate(this.id)" onclick="styleback(this.id)" type="text" class="xoouserultra-input"  name="'.$meta.'" id="'.$meta.'" value="'.$this->get_post_value($meta).'"  title="" />';

							$help="Include [+][Countrycode][PhoneNo.] Ex: +919000000000";
								$display .= '<div id="'.$meta.'-help" class="xoouserultra-help">'.$help.'</div><div class="xoouserultra-clear"></div><br>';
							}
							else
							{

							$display .= '<input placeholder="'.$name.'" type="text" class="'.$required_class.'xoouserultra-input"  name="'.$meta.'" id="'.$meta.'" value="'.$this->get_post_value($meta).'"  title="'.$name.'" data-errormessage-value-missing="'.__(' * This input is required!','xoousers').'"/>';
							}
							break;							
    ...
    <label id="'.$meta.'_label" class="xoouserultra-field-type"></label><div id="'.$meta.'_err" class="alert alert-danger xoouserultra-field-value"></div>
		...
		$display .='<div class="xoouserultra-field xoouserultra-edit xoouserultra-edit-show">Got a Coupon/Promo Code? <div id="cc_div_show" style="display:block"><label class="xoouserultra-field-type" for="cc"><i class="fa fa-tags" aria-hidden="true"></i><span>Enter Coupon/Promo Code </span></label><div class="xoouserultra-field-value"><input onkeypress="return event.charCode !=32" onblur="return validate(this.id)" onclick="return styleback(this.id)" type="text" class="xoouserultra-input" name="cc" id="cc" value="" title=""><div id="cc_err_div" class="xoouserultra-field" style="display:none"><label id="cc_label" class="xoouserultra-field-type"></label><div id="cc_err" class="alert alert-danger xoouserultra-field-value"></div> </div></div><div class="xoouserultra-help">To get a promocode, visit <a target="_blank" href="http://www.facebook.com/QezyPlay">QezyPlay Facebook page</a></div><div class="xoouserultra-clear"></div><br></div></div>';

		$display .='<script>
				function cc_form(){
				var ok=jQuery(\'#cc_div_show\').css("display");
				if(ok=="none") {
					
					jQuery(\'#cc_div_show\').html(\'<label class="xoouserultra-field-type" for="cc"><i class="fa fa-tags" aria-hidden="true"></i><span>Enter Coupon/Promo Code </span></label><div class="xoouserultra-field-value"><input onkeypress="return event.charCode !=32" onblur="return validate(this.id)" onclick="return styleback(this.id)" type="text" class="xoouserultra-input" name="cc" id="cc" value="" title=""><div id="cc_err_div" class="xoouserultra-field" style="display:none"><label id="cc_label" class="xoouserultra-field-type"></label><div id="cc_err" class="alert alert-danger xoouserultra-field-value"></div> </div><a href="javascript:void(0)" onclick="return cc_div_hide()">No Coupon/Promo</a></div><div class="xoouserultra-help">To get a promocode, visit <a href="http://www.facebook.com/QezyPlay">QezyPlay Facebook page</a></div><div class="xoouserultra-clear"></div><br>\');

					jQuery(\'#cc_div_show\').css("display","block");
					jQuery(\'#coupon\').val("true");	} else {jQuery(\'#cc_div_show\').html(\'\');
						jQuery(\'#cc_div_show\').hide();
					jQuery(\'#coupon\').val("false");}}

				function cc_div_hide(){jQuery(\'#cc_div_show\').hide();}
			</script>';
		$display .= '<div class="xoouserultra-field xoouserultra-edit xoouserultra-edit-show">
		
						<div class="xoouserultra-field-value">
						    <input type="hidden" name="xoouserultra-register-form" value="xoouserultra-register-form" />
                  <div style="color:black" id="tos_pp"><input onchange="styleback(this.id)" onblur="return validate(this.id)" style="position:inherit" type="checkbox"  title="'.__('', 'xoousers').'" name="uultra-terms-and-conditions-confirmation1" id="uultra-terms-and-conditions-confirmation1" value="1" class="" > By clicking on Register, you agree to our <a href="../terms-of-service">Terms of Service</a> and that you have read our <a href="../privacy-policy">Privacy Policy</a></input> </div>
<br />
<div id="uultra-terms-and-conditions-confirmation1_err_div" class="xoouserultra-field" style="display:none">
    <label id="uultra-terms-and-conditions-confirmation1_label" class="xoouserultra-field-type"></label><div id="uultra-terms-and-conditions-confirmation1_err" class="alert alert-danger xoouserultra-field-value"></div>
   </div>	<input type="hidden" id="coupon" name="coupon" value="false"></input>

					<input  onclick="return validate(this.id)" type="submit" name="xoouserultra-register" id="xoouserultra-register-btn" class="btn btn-success" value="'.__('Register','xoousers').'" />
					<!--<input  onclick="return validate(this.id)" type="button" name="xoouserultra-register" id="xoouserultra-register-btn" class="btn btn-success" value="'.__('Register','xoousers').'" />--><!--xoouserultra-button-->
						</div>
					</div><div class="xoouserultra-clear"></div><br><br>';
...
$( "#reg_user_login" ).click(function() {
 $(".usersultra-errors").hide();
});
$( "#reg_user_email" ).click(function() {
 $(".usersultra-errors").hide();
});
if(clear=="ok"){reset_fn();}
function reset_fn()
{
...
function avoidspace(id)
{
	...
	public function get_social_buttons ($action_text, $atts)
	{
		...
		if($this->get_option('registration_rules')!=4) // Social media is not able when using paid registrations
		{
			...
			$divider .='<div align="center" style="color:black;font-size:12px;margin:0 auto;"> *By Sign-in or Sign-up with Facebook, 				you agree to our <a href="../terms-of-service">Terms of Service</a> and that you have read our <a 				href="../privacy-policy">Privacy Policy</a></div>';
			...
			if($this->get_option('social_media_fb_active')==1)
			{
				...
				//Facebook
				$display .=$divider.'<div class="txt-center FacebookSignIn">
