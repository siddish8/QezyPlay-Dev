...
/*Process uploads*/
	...
	public function send_reset_link()
	{
		...
		if(is_email($_POST['user_login']))
		{
			  ...
			  {
				  //user not found
				  $html ="<div class='uupublic-ultra-error'>".__('Email not foundd','xoousers')."</div>";
			  	$html.="<script>swal('check');</script>";
			  }else{
				  ...
				  //user found 				  
				   ...
				   {					   					   
					   //user is not active					   
					   $html ="<div class='uupublic-ultra-error'>".__('Your account is not active yet.','xoousers')."</div>";				   
					   $noactive = true;
			  ...
			  {
				  //user not found
				  $html ="<div class='uupublic-ultra-error'>".__('Please enter username or e-mail','xoousers')."</div>";
			...
			if ( $type == 'usermeta' && $deleted == 0 && $private == 0)			
			 {
				...
				$html .= '<div class="xoouserultra-field-value">';
					...
					$html.='<input type="text" maxlength="10" class="xoouserultra-input'.$required_class.'" 					name="'.$meta.'" id="'.$meta.'" value="'.$this->get_user_meta( $meta).'"  title="'.$name.'" 						'.$disabled.' /></div></div><div class="xoouserultra-clear"></div>';
					continue;}
				...
				if ($can_edit == 0)
				{
					...
					switch($field) {
case 'fileupload':
							$html.='<input style="display:block !important;" type="file" class="xoouserultra-input'. $required_class.'" name="'.$meta.'" id="'.$meta.'" value="'.$this->get_user_meta( $meta).'"  title="'.$name.'" '.$disabled.' >';
							break;
	...
	function update_me() 
	{
        ...
        $date_time_fields = array();$_SESSION['errMsg'] = "";
        ...
        foreach ($array as $key => $field) {
            ...
            if (isset($array[$key]['field']) && $array[$key]['field'] == 'fileupload') 
			{
				...
				&& $imageFileType != "gif" ) {
					...
					$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/wp-content/uploads/'.$upload_folder."/".$user_id."/";
	...
	public function  display_optional_fields3($user_id, $display_country_flag, $fields_to_display)	
	{
		...
		foreach($array as $key => $field) 
		{
			...
			{				
				...
				if (isset($array[$key]['name']) && $name)
				{
					if($name=="Profile Pic"); else $html .= ' <span class="data-a">'.$name.'</span><span class="data-b"><strong>'.$this->get_custom_user_meta( $meta, $user_id).'</strong></span> ';
				}
