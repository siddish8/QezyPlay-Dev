...
require_once $_SERVER['DOCUMENT_ROOT'].'/newqezyplay/mobile_detect/Mobile_Detect.php'; //DEMO
		...
		if($detect->isMobile()){
			if($detect->isAndroidOS()){
					//Android
				global $wpdb;
				//$action=$wpdb->get_var("select action from androidQP order by id desc limit 1");
				//$version=$wpdb->get_var("select version from androidQP order by id desc limit 1");				
					$action=2;
					$version='2.0.1';
					echo "<script> 						
						var action=".$action.";
						var version='".$version."';
						Android.versionCheck(version,action); 
							/* alert(arg); */
						</script>";

					
					//echo '<img onload="androidQP(\'intent:#Intent;action=com.ib.qezyplay;S.check=\'yes\';S.action='.$action.';S.version='.$version.';package=com.ib.qezyplay;end\')">';

				//var arg=\"intent:#Intent;action=com.ib.qezyplay;S.check='yes';S.action='".$action."';S.version='".$version."';package=com.ib.qezyplay;end\";
}
}
