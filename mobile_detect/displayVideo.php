...
function displayVideo($a,$b,$c,$d,$e,$f,$g,$h,$i,$j,$k,$l,$m,$n,$o,$p,$q,$r){
	...
	echo "<script>console.log(2+'".$vodurl."')</script>";
	echo "<script>console.log(2+'".$vodturl."')</script>";
...
$ok=validateAuthToken($access_token);

echo "<script>console.log('Sending Access_Token:".$access_token."')</script>";//echo "Sending Token: ".$ready_token;

//echo "<script>console.log('Access_Token ok:".$ok."')</script>";//echo "Sending Token: ".$ready_token;
...
//if (($sub_status=="active") or ( (int)$subAgent>0 and $validity==1) or ($user_id==29) ){
	...
	//if(is_user_logged_in()){
		...
		if($detect->isMobile()){
			...
			if($detect->isAndroidOS()){
				...
				//echo '<script>swal("'.$_SERVER['HTTP_X_REQUESTED_WITH'].'")</script>';
				...
				echo '<div align="center" id="player_img" style="background-image:url('.$imgurl.') !important;background-size: 100% 100% !important;width:650px;height:360px;background-color: white !important;border-color: black ;color: Orange;" align="center"><img src="'.site_url().'/wp-content/uploads/2016/06/PlayImg.png" onclick=""></img><div>';
				...
				echo '<div align="center" id="player_img" style="margin:0 auto;background-image:url('.$imgurl.') !important;background-size: 100% 100% !important;width:650px;height:360px;background-color: white !important;border-color: black ;color: Orange;" align="center"><img src="'.site_url().'/wp-content/uploads/2016/06/PlayImg.png" onclick="swal(\'Sorry for Inconvenience\',\'  Currently we are not having the supporting player for IOS , Will be publised very soon.\')"></img><div>';
//onclick="playinIosApp(\'iosQezyplayer://myapp?user_id='.$user_id.'&access_token='.$access_token.'\')"
		...
		//$vidurl="octoshape://streams.octoshape.net/ideabytes/live/ib-ch60/auto";
		//$vodurl="octoshape://streams.octoshape.net/ideabytes/vod/IoT-Conference/Event-1.flv";
 ...
 <script type="text/javascript">// <![CDATA[
		var streamVID = \''.$vidurl.'\';
		var streamVOD = \''.$vodurl.'\';// ]]></script> 
...
//echo '<script src="'.site_url().'/players/octoshape/player.js" type="text/javascript"></script>';
		...
		//echo "<script>swal('subscribed_".$user_id.":".get_option("subscribed_".$user_id,"")."')</script>";
		...
		if($detect->isAndroidOS()){
				...
				echo '<script>	
					...
					playinAndroidApp(\'intent:#Intent;action=com.ideabytes.qezyplay.LAUNCH_PLAYER;S.page_id='.$post_id.';S.post_title='.$post_title.';S.post_name='.$post_name.';S.user_id='.$user_id.';S.user_name='.$user_name.';S.page_url='.$page_url.';S.page_referer='.$page_referer.';S.browser_name='.$browser_name.';S.browser_version='.$browser_version.';S.session_id='.$session_id.';S.country_code='.$country_code.';S.country='.$country_name.';S.state='.$state.';S.city='.$city.';S.geo_info_status='.$geoinfo_status.';S.imgpath='.$imgurl.';S.octo_url='.$vodturl.';S.app_analytics_update_duration='.$app_analytics_update_duration.';S.app_android_version='.$app_android_version.';S.app_force_update_status='.$app_force_update_status.';S.app_shutdown_wait_timeout='.$app_shutdown_wait_timeout.';package=com.ib.qezyplay;end\');
