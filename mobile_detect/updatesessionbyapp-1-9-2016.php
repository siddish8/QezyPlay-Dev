...
//REMOVE BELOW (only for testing) LOCAL dbcon Added on 27-04-2016
	...
	if($user_id > 0){
		...
		if($stmt1->rowCount() > 0){
			...
			$sql = "INSERT INTO visitors_info(user_id,user_name,user_email,ip_address,page_id,page_name,page_referer,country_code,country,state,city,geo_info_status,device,os_name,os_version,browser_name,browser_version,start_datetime,end_datetime,session_id,duration,play) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";		
			$data = array($user_id,$user_name,$user_email,$ip_address,$page_id,$page_name,$page_referer,$country_code,$country,$state,$city,$geoinfo_status,$device,$os_name,$os_version,$browser_name,$browser_version,$start_time,$end_time,$session_id,$duration,$play);
		}
		$stmt = null;
