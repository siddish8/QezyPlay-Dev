...
if((int)$_SESSION['adminid'] <= 0){
	...
	$sql1 = "SELECT count(*) as count,a.page_id,b.post_title FROM visitors_info a inner join wp_posts b on b.ID=a.page_id WHERE 1 and b.post_status='publish' and b.post_type='post'".$cond." GROUP BY a.page_id";	
	$stmt1 = $dbcon->prepare($sql1, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
	$stmt1->execute();
	$hits = $stmt1->fetchAll(PDO::FETCH_ASSOC);

	$sql2 = "SELECT sum(duration) as duration,a.page_id,b.post_title FROM visitors_info a inner join wp_posts b on b.ID=a.page_id WHERE 1 AND duration > 0 AND play = 1 and b.post_status='publish' and b.post_type='post'".$cond." GROUP BY page_id";	
	...
	<div id="content" role="main" style="min-height:500px;margin: 2% 5%;">
		...
		<div class="pmpro_box" id="pmpro_account-invoices">
			<h2 style="text-align:center">Statistics By Channel</h2>
