...
switch($action){
	...
	case "couponCheck":
		...
		$promo_cc_exist=get_var("SELECT count(*) from promo_codes where coupon_code='".$cpn."'");
			...
			$promo_cc_max_used=get_var("SELECT count(*) from promo_codes where usage_count<max_count and coupon_code='".$cpn."'");
				if($cc_used==0 and $promo_cc_max_used==0)
				{
				$msg="2";
				//$msg="Coupon Code already used"; or max_used
				echo $msg;
				}
				else{
				$msg="3";
...
//added
	...
	case "lr":
		...
		$_SESSION['log_href']=$value="/buy-a-friend?plan_id=".$plan."&boq_id=4"; 
	...
	case "regEditEmail":
		...
		*/
		...
		//if(get_var("SELECT meta_value FROM wp_usermeta where meta_key='usersultra_account_status' and user_id=(SELECT ID from wp_users where user_email='".$o_email."')")=="active")
			if(get_var("SELECT meta_value FROM wp_usermeta where meta_key='usersultra_account_status' and user_id=(SELECT ID from wp_users where user_email='".$o_email."')")!="pending")
			{
				echo "Your Account Already Activated"; return; exit;
		...
		if(get_var("SELECT count(*) from wp_users where user_email='".$email."'")>0 or !preg_match("/^[a-zA-Z0-9.!#$%&Â’*+\/=?^_`{|}~-]+@[a-zA-Z0-9-]+[\.].[a-zA-Z0-9]+(?:\.[a-zA-Z0-9-]+)*$/", $email) or $email=="")
			...
			
			{
				...
				echo "Email-id already exists. Choose Another"; return; exit;
			}
		...
		//else
			//{
				...
				//fwrite($file,$info);
				echo "Email Edited to $email and Activation Link Resent"; return;
		...
		*/
		...
		if(get_var("SELECT count(*) from wp_users where user_email='".$email."'")>0 or !preg_match("/^[a-zA-Z0-9.!#$%&Â’*+\/=?^_`{|}~-]+@[a-zA-Z0-9-]+[\.].[a-zA-Z0-9]+(?:\.[a-zA-Z0-9-]+)*$/", $email) or $email=="")
			...
			
			{
				...
				echo "<p><b>Error</b></p><p>Email-id already exists. Choose Another</p>"; return; exit;
			}
		...
		//else
			//{
				...
				//$info="Email Edited to ".$email." and Activation Link Resent";
				//fwrite($file,$info);
				echo "<p><b>Info</b></p><p>Email Edited to $email and Activation Link Resent</p>"; return;
	...
	case "resendEmail":
				...
				if(get_var("SELECT meta_value FROM wp_usermeta where meta_key='usersultra_account_status' and user_id=(SELECT ID from wp_users where user_email='".$email."')")!="pending")
			{
				echo "Your Account Already Activated"; return; exit;
			...
			{				
				...
				//fwrite($file,$info);
				echo "Activation Email has been sent to your Registered Email-id "; return;
			}
			exit;
		break;
	...
	case "setStreamStatus":
		...
		if($status=="0")
		{
		//mail("siddish.gollapelli@ideabytes.com","error $channel_name",print_r("there is an error $channel_id, please check",true));
		echo "mail sent";
		}
...
function  re_send_activation_link($u_email, $user_login, $activation_link,$user_id,$subject_admin)
	{
		...
		$delay=get_var("SELECT delay from pmpro_dates_chk1 where user_id=$user_id order by id desc limit 1");
		...
		//$template_admim = stripslashes($this->get_option('messaging_welcome_email_with_activation_admin')); <p> Password: {{userultra_pass}} </p>
			...
			$template_client="<p>Hi,</p> <p>Thanks for registering. Your account needs activation.</p><p> Please click on the link below to activate your account: {{user_ultra_activation_url}} </p><p>E-mail: {{userultra_user_email}} </p><p>Username: {{userultra_user_name}}</p><p>Note: $sub_stat Enjoy Viewing.</p><p>You can change your password at {{userultra_change_password}} after Login.<br><p>If you have any problems, please contact us at {{userultra_admin_email}}.</p>  <p>Best Regards, </p><p>Admin - Qezyplay</p>";
