...
if((int)$_SESSION['customerid'] <= 0){
	...
	<div id="content" role="main" style="min-height:500px;margin: 2% 5%;">
		...
		<div class="pmpro_box" id="pmpro_account-invoices">
			<h2 style="text-align:center">Statistics By Device</h2>
	...
	<?php if($startdate!="" && $enddate!=""){ 
		...
		if(count($hits) > 0){ 
			...
			$hitsdata = "";
			foreach($hits as $hit){
				$hitsdata .= '{device:"'.$hit["os_name"].'('.$hit["device"].')",value:"'.$hit["count"].'"},';		
			}
		...
		if(count($durations) > 0){ 
			...
			foreach($durations as $duration){
				...
				$durationsdata .= '{device:"'.$duration["os_name"].'('.$duration["device"].')",value:"'.$playedduration.'"},';		
			}
