...
class WP_Query {
	...
	protected function parse_search( &$q ) {
		...
		foreach ( $q['search_terms'] as $term ) {
			...
			$search .= $wpdb->prepare( "{$searchand}(($wpdb->posts.post_title $like_op %s) $andor_op ($wpdb->posts.post_excerpt $like_op %s) $andor_op ($wpdb->posts.post_content $like_op %s)) OR ((replace($wpdb->posts.post_title , ' ','') $like_op %s) $andor_op (replace($wpdb->posts.post_excerpt  , ' ','') $like_op %s) $andor_op (replace($wpdb->posts.post_content  , ' ','') $like_op %s))", $like, $like, $like, $like, $like, $like );
			$searchand = ' AND ';
		}
